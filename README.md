...
1. 首轮必须包含方块 3，且持有方块 3 的玩家先手。
2. 点数相同时继续比较花色，花色顺序为方块 < 梅花 < 红桃 < 黑桃。
3. 支持 `A2345` 顺子，最大点数按 5 处理。
...
4. `RoomState` 会同步已加入玩家、已准备玩家和需要蓝牙连接的真人座位；真人座位全部准备后房主自动开局。
