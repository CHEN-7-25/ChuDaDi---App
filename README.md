...
7. 本地人机对战界面（1 名玩家 + 3 名 AI，支持选牌、出牌、过牌、提示、20 秒回合倒计时、连续对局与累计比分）
8. 蓝牙联机框架（房间、座位、准备、开局 seed、快照、私人手牌、心跳与手动重连）
9. 背景音乐，可选择不同风格的背景音乐，提供一键静音。
10. 个人信息，可自定义昵称，查看总局数，胜局数，胜率等等。
...
- 过牌与重开逻辑。
- 任意玩家手牌出完后本局立即结束；默认积分为赢家 +3，其余玩家按剩余手牌数从少到多补齐名次并获得 +1 / -1 / -3。
...
13. 每个玩家回合有 20 秒倒计时；超时后可过牌时自动过牌，不可过牌时自动选择一手合法牌，联机时由房主维护权威状态并同步给客户端。
14. 任意未出完玩家只剩 1 张牌时，牌桌提示和头像状态会显示“报单”提醒；该提醒不改变出牌规则。
...
可在 Android Studio 中运行 `test` 任务；命令行可执行：

```bash
gradle clean :app:testDebugUnitTest :app:testReleaseUnitTest :app:assembleDebug
```

当前 Debug / Release 单元测试各 35 个用例，已覆盖规则、控制器、蓝牙消息编解码和牌面编码。

## 9. 文档入口
- 文档总览：[Documents/README.md](Documents/README.md)
- 项目总体架构：[Documents/项目总体架构说明.md](Documents/项目总体架构说明.md)
